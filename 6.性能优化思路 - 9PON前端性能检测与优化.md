...
## 四、指标上报时机
| 指标   | 最佳上报时机                                                                 | 兜底上报时机                                                                 | 最佳上报时机原因                                                                 |
|--------|------------------------------------------------------------------------------|------------------------------------------------------------------------------|----------------------------------------------------------------------------------|
| LCP    | 1. 指标最终稳定时（`metric.isFinal === true`）；<br>2. 避免频繁上报，仅在最终值触发时上报 | 页面 `visibilitychange` 触发 `hidden` 状态时（切后台/卸载前）| 1. LCP 会动态更新（如图片加载完成后值会变化），`isFinal` 为 `true` 时才是最终值；<br>2. 兜底避免最终值未触发时数据丢失 |
| INP    | 页面 `visibilitychange` 触发 `hidden` 状态时（切后台/卸载前）| 页面 `beforeunload` 事件触发时（极端场景兜底）| 1. INP 需要采样页面生命周期内所有用户交互，只有页面卸载前才能确定 95 百分位最终值；<br>2. `visibilitychange` 比 `beforeunload` 执行更早，上报成功率更高 |
| CLS    | 1. 每次布局偏移后（增量上报，可选）；<br>2. 页面 `visibilitychange` 触发 `hidden` 状态时（上报最终累积值） | 页面 `beforeunload` 事件触发时（补充上报最终值）| 1. 增量上报可监控实时布局偏移，便于定位高频移位场景；<br>2. 卸载前上报最终总分，保证累积值完整；<br>3. CLS 是累积值，需覆盖页面全生命周期 |
| FCP    | 指标计算完成时（`web-vitals` 回调触发时）| 页面 `visibilitychange` 触发 `hidden` 状态时（兜底，防止回调未触发）| FCP 只会触发一次（页面首次渲染有意义内容时），计算完成后立即上报即可，兜底仅为防止极端场景下回调丢失 |
| FP     | 指标计算完成时（`web-vitals` 回调触发时）| 页面 `visibilitychange` 触发 `hidden` 状态时（兜底，防止回调未触发）| FP 只会触发一次（页面首次绘制像素时），计算完成后立即上报，兜底覆盖极端场景 |
| FID    | 指标计算完成时（`web-vitals` 回调触发时）| 页面 `visibilitychange` 触发 `hidden` 状态时（兜底，防止回调未触发）| FID 只会触发一次（首次用户交互的延迟），计算完成后立即上报，兜底避免数据丢失（FID 已逐步被 INP 替代） |

- 补充说明（表格外的关键细节）
  - 兜底时机优先级：visibilitychange（hidden 状态）＞ beforeunload ＞ unload（不推荐）；
  - 重复上报规避：需加标记（如 isReported 变量），避免兜底时机和最佳时机重复上报同一指标；
  - 上报方式：兜底时机建议用 navigator.sendBeacon（优先）或 fetch keepalive: true，保证页面卸载时请求不中断。


### 使用方式
- #### 1. LCP（最大内容绘制）—— 需判断 isFinal
```js
import { getLCP } from 'web-vitals';

// LCP 回调（多次触发，仅最终值上报）
getLCP((metric) => {
  // 仅当 isFinal 为 true 时，才是最终值（最佳上报时机）
  if (metric.isFinal) {
    reportMetric({
      name: metric.name,
      value: metric.value, // 单位：ms
      id: metric.id, // 唯一ID，避免重复上报
      isFinal: metric.isFinal
    });
  }
});

// 兜底：页面卸载前补充上报（防止 isFinal 未触发）
document.addEventListener('visibilitychange', () => {
  if (document.visibilityState === 'hidden') {
    // 可额外获取当前 LCP 值兜底上报（需结合闭包存储）
  }
});
```
...
// INP 回调仅在页面卸载前触发一次（自动计算 95 百分位值）
getINP((metric) => {
  // 直接上报即可（无需判断，回调触发时已是最终值）
  reportMetric({
    name: metric.name,
    value: metric.value, // 单位：ms
    id: metric.id
...
- #### 3. CLS 回调函数的核心使用方式
CLS 是累积型指标，回调会在每次布局偏移发生时触发，核心用法分两种：「增量上报（实时监控）」+「最终值上报（页面卸载前）」，需结合 visibilitychange 兜底：
```js
...
// 2. CLS 回调函数（每次布局偏移触发）
getCLS((metric) => {
  // 更新累积值（metric.value 是当前总 CLS）
  totalCLS = metric.value;
  
  // 方式1：增量上报（可选，用于实时监控单次偏移）
  console.log('单次 CLS 增量：', metric.delta); // 本次偏移的增量值
  console.log('当前 CLS 累计值：', metric.value);
  reportMetric({
    name: metric.name,
    value: metric.value,
    delta: metric.delta,
    type: 'cls-increment' // 标记增量上报
  });
});

// 3. 页面卸载/切后台时，上报 CLS 最终值（核心）
let isCLSReported = false;
document.addEventListener('visibilitychange', () => {
  if (document.visibilityState === 'hidden' && !isCLSReported) {
    // 方式2：最终值上报（推荐，保证数据完整）
    reportMetric({
      name: 'CLS',
      value: totalCLS,
      type: 'cls-final' // 标记最终值
    });
    isCLSReported = true;
  }
});
...
- CLS 回调关键说明：
  - metric.value：当前累积的总 CLS 值（每次回调都会更新）；
  - metric.delta：本次布局偏移的增量值（单次要因分析用）；
  - 无需判断 isFinal：CLS 的 isFinal 仅在页面卸载前由库标记，直接用 visibilitychange 上报最终值更可靠。
 
- #### 4. FCP/FP/FID（一次性指标）—— 回调触发即上报
```js
import { getFCP, getFP, getFID } from 'web-vitals';
...
#### 所有指标回调使用总结表
| 指标         | 回调触发次数       | 核心使用逻辑                                  | 关键注意点                              |
|--------------|--------------------|-----------------------------------------------|-----------------------------------------|
| CLS          | 多次（每次偏移）| 实时更新累积值，卸载前上报最终值              | 用 `metric.value` 取累计值，`delta` 取单次增量 |
| LCP          | 多次（元素变化）| 仅 `metric.isFinal === true` 时上报最终值     | 需兜底防止最终值未触发                  |
| INP          | 1次（卸载前）| 回调触发直接上报（自动计算 95 百分位）        | 优先依赖回调，无需额外判断              |
| FCP/FP/FID   | 1次（计算完成）| 回调触发直接上报                              | 无 `isFinal` 属性，无需判断             |

### 四、通用最佳实践
1. **去重**：利用 `metric.id` 标记已上报的指标，避免重复；
2. **兜底**：所有指标都建议在 `visibilitychange` 的 `hidden` 状态补充上报；
3. **上报方式**：卸载前的上报优先用 `navigator.sendBeacon`，普通上报用 `fetch`/`axios`；
4. **异常过滤**：上报前过滤异常值（如 LCP > 30s、CLS > 5），保证数据有效性。
